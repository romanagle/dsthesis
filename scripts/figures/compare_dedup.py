...
parser.add_argument("original", nargs="?", default=f"{DATA_ROOT}/trnascan_GTDB_r226_v2_final.csv")
parser.add_argument("dedup",    nargs="?", default=f"{DATA_ROOT}/data/dedup/trnascan_GTDB_r226_dedup.csv")
args = parser.parse_args()

OUT = Path(f"{DATA_ROOT}/figures/dedup_comparison")
...
def fig_overview():
    ...
    ax.set_ylabel("Count")
    ax.set_title("Effect of deduplication on tRNA dataset size")
    ax.legend()
    plt.tight_layout()
