"""
EDA visualizations for trnascan_GTDB_r226_v2_final.csv
...
from utils.iso_classes import ISOTYPE_ORDER, remap_isotypes


# ── CLI ───────────────────────────────────────────────────────────────────────
parser = argparse.ArgumentParser(description="EDA for trnascan GTDB CSV")
parser.add_argument("csv", nargs="?", default=f"{DATA_ROOT}/trnascan_GTDB_r226_v2_final.csv",
                    help="Path to input CSV (default: trnascan_GTDB_r226_v2_final.csv)")
args = parser.parse_args()

DATA = args.csv
OUT  = Path(f"{DATA_ROOT}/figures") / Path(DATA).stem
OUT.mkdir(parents=True, exist_ok=True)
print(f"Input:  {DATA}")
print(f"Output: {OUT}/")

AA_SCORE_COLS = list(ISOTYPE_ORDER)
