...
Produces:
  figures/fig1_accuracy_summary.png   — CV + test accuracy bar chart
  figures/fig2_per_class_f1.png       — per-class F1 heatmap
  figures/fig3_precision_recall.png   — per-class precision & recall grouped bars
  figures/fig4_confusion_matrices.png — 2×2 grid of test-set confusion matrices
"""
...
from utils.iso_classes import ISOTYPE_ORDER as AMINO_ACIDS
...
# ── helpers ──────────────────────────────────────────────────────────────────
CSV_PATH = "runs/isomodels_first100_with_test.csv"
...
ax.set_ylim(0, 1.05)
ax.axhline(1/len(AMINO_ACIDS), color="grey", linestyle="--", linewidth=0.8,
           label=f"Random (1/{len(AMINO_ACIDS)})")
...
def run_onehot_classifier(masked=False):
    ...
    if masked:
        mdf = pd.read_csv("runs/isomodels_first100_anticodon_masked.csv")
