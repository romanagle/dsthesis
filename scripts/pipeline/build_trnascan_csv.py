...
def parse_struct(path: str) -> dict:
    ...
    Primary sequence = full Seq: line with case preserved. Lowercase letters indicate
    insertions (introns, variable loop extensions) that have no corresponding position
    in the dot-bracket secondary structure.
    """
    structs: dict = {}
    current_key = None
    current_seq = None
    with open(path, encoding="utf-8") as f:
        for line in f:
            ...
            if "Length:" in line:
                ...
                # Strip genomically-encoded CCA tail; SS string never includes it
                if raw.upper().endswith("CCA"):
                    raw = raw[:-3]
                current_seq = raw
