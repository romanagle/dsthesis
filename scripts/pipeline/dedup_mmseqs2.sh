...
# then filters v2_final_matched.csv to keep one representative row per
# unique sequence.
#
# Output: $DATA_ROOT/data/dedup/trnascan_GTDB_r226_dedup.csv
#
# Usage:  bash scripts/dedup_mmseqs2.sh
# ─────────────────────────────────────────────────────────────────────────────

set -euo pipefail

# ── config ────────────────────────────────────────────────────────────────────
CSV="$DATA_ROOT/trnascan_GTDB_r226_v2_final_matched.csv"
WORKDIR="$DATA_ROOT/mmseqs2_dedup"
...
repr_ids_path = "$REPR_IDS"
csv_in_path   = "$CSV"
csv_out_path  = "$DATA_ROOT/data/dedup/trnascan_GTDB_r226_dedup.csv"
